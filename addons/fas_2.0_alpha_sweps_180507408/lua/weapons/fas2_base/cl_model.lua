			...
			if self.MagCheck then
				...
				if self.FireMode == "safe" then
					...
					if self.MoveType == 1 then
						TargetPos[1] = math.Approach(TargetPos[1], (self.SprintPos and self.SprintPos[1] or 4), FT * 25 + math.Clamp(tan or 0, -0.4, 0.1))
						TargetPos[2] = math.Approach(TargetPos[2], (self.SprintPos and self.SprintPos[2] or -2.09), FT * 25 + math.Clamp(cos1 or 0, 0, 0.2))
						TargetPos[3] = math.Approach(TargetPos[3], (self.SprintPos and self.SprintPos[3] or -0.24), FT * 25 + math.Clamp(cos1 or 0, 0, 0.4))
...
function SWEP:DrawWorldModel()
	...
	if IsValid(wm) then
		if IsValid(self.Owner) then
			...
			pos, ang = GetBonePosition(self.Owner, boneID)
