...
function SWEP:Deploy()
	...
	timer.Simple( 0, function()
		...
		if self.m_bAmmoHack then
			self.m_bAmmoHack = nil
			self:SetClip1( self:Clip1() -1 )
		end
	end )
...
function SWEP:PrimaryAttack()
	if self.m_bAmmoHack then return end
...
function SWEP:OnRemove()
	if CLIENT then
		...
		if IsValid( self.Owner ) then
			self.Owner:GiveAmmo( math.max(self:Clip1() -(self.m_bAmmoHack and 2 or 1), 0), self:GetPrimaryAmmoType() )
		end
	end
...
function SWEP:OnDrop( ... )
	...
	if IsValid( self.m_pLastOwner ) and self.m_pLastOwner:Alive() then
		self.m_pLastOwner:GiveAmmo( self:Clip1(), self.Primary.Ammo )
		self:SetClip1( 0 )
	end
end