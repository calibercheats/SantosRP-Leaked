...
function ENT:OnButton(btn)
	if btn.x < self.menu.curser.x and btn.x + btn.width > self.menu.curser.x and btn.y < self.menu.curser.y and btn.y + btn.height > self.menu.curser.y then
...
function ENT:Page(forward)
	...
	if self.menu.page == 1 then
		self.menu.page = self.menu.pages
		return
	end
	self.menu.page = self.menu.page - 1
end
	...
	cam.Start3D2D( OffSet, Ang, Scale )
		...
		if trace.Entity == self then
			...
			if localVect.y > (-8.3) and localVect.y < 93 and localVect.z > (-64) and localVect.z < 5 then
				self.menu.curser = { x = localVect.y, y = localVect.z*(-1) }
				--surface.DrawRect(self.menu.curser.x,self.menu.curser.y,5,5)
		...
		if self.menu.page == 1 then
			...
			for k, v in pairs(self.connected_radios) do
				...
				y = y + 10
				draw.DrawText( string.sub(Entity(v):GetSpeaker():GetName(), 1, 13), "BudgetLabel", 0, y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			end
...
function ENT:Think()
	if SERVER then
		...
		if IsValid(own_speaker) and own_speaker:IsPlayer() and own_sending and !own_speaker.ChatGesture then
			if !own_speaker.ChatStation then own_speaker.ChatStation = true end
			own_speaker.ChatGesture = true
		end
		local distance, distance_max, radio_pos = gspeak:get_distances(self, 2)
