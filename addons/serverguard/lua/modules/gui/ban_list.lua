...
category.permissions = {"Ban", "Permaban", "Unban"};
category.loading = {
	firstRun = true,
	inProgress = false,
	banTable = {},
...
function category:Think(base)
	...
	for i = category.loading.currentIndex, category.loading.currentIndex + 4 do
		...
		if (IsValid(category.list)) then
			local panel = category.list:AddItem(i, name, steamID, lengthText, admin, reason);
			...
			if (i == #category.loading.banList) then
...
serverguard.netstream.Hook("sgGetBanListChunk", function(data)
	if (type(data) == "string") then
		category.loading.banList[#category.loading.banList + 1] = data;

		serverguard.netstream.Start("sgRequestBanListChunk", true);
	elseif (type(data) == "boolean" and data == true) then
		...
		if (category.loading) then
			...
			category.loading.banList = serverguard.netstream.Build(category.loading.banList);
