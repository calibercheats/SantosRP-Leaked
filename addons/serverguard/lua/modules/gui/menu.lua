...
concommand.Add("+serverguard_quickmenu", function(player)
	if (serverguard.player:HasPermission(player, "Quick Menu")) then
		...
		local menu = DermaMenu();
			...
			for k, v in pairs(util.GetSortedPlayers()) do
				...
				for k2, v2 in util.SortedPairsByMemberValue(commandsTable, "command") do
					if (v2.ContextMenu and (!v2.permissions or serverguard.player:HasPermission(player, v2.permissions))) then
