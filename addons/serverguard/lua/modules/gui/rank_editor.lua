...
function category:Create(base)
	...
	base.panel.list:AddColumn("UNIQUE", 240)
	base.panel.list:AddColumn("NAME", 240)
	base.panel.list:AddColumn("IMMUNITY", 100)

	hook.Call("serverguard.panel.RankEditorList", nil, base.panel.list);
	
	local column = base.panel.list:AddColumn("IMAGE", 90)
...
function category:Update(base)
	...
	for unique, data in pairs(ranks) do
		local panel = base.panel.list:AddItem(unique, data.name, data.immunity)
		...
		function panel.icon:PerformLayout()
			...
			local column = panel:GetThing(4).column;
