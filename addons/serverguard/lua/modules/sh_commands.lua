...
if (SERVER) then
	...
	hook.Add("PlayerSay", "serverguard.command.PlayerSay", function(player, text, m_bToAll, m_bDead)
		...
		if (prefix == "!" or prefix == "~") then
			...
			if (player.sgMuted and commandName != "unmute") then
				return "";
			end;

			local commandExists = serverguard.command.Run(player, commandName, bSilent, unpack(arguments));
			
			if (commandExists) then
				return "";
			end;
		elseif (player.sgMuted) then
			return "";
		end;
	end);

	serverguard.netstream.Hook("sgRunCommand", function(player, data)
		serverguard.command.Run(player, data.command, data.silent, unpack(data.arguments));
	end);

	concommand.Add("sg", function(player, command, arguments)
		if (arguments and arguments[1]) then
			...
			if (commandTable) then
				serverguard.command.Run(player, commandTable.command, false, unpack(arguments));
			end;
		end;
	end);
elseif (CLIENT) then
	...
	concommand.Add("sg", function(player, command, arguments)
		...
		if (arguments[1]) then
			...
			if (command and serverguard.player:HasPermission(LocalPlayer(), command.permissions)) then
				table.insert(results, "sg " .. command.command .. " " .. serverguard.command:GetArgumentsString(command));
