...
function serverguard.player:SetRank(player, rank, bSaveless)
	...
	if (type(player) == "string" && string.SteamID(player)) then
	...
	player:SetNetworkedString("serverguard_rank", rank);
...
--- Returns whether or not the player has a better immunity level than the one specified.
-- @player player The player to check the immunity level of.
...
function serverguard.player:HasBetterImmunity(player, immunity)
	return self:GetImmunity(player) > immunity;
end;

--- Gets a player's name. Will return with the orignal name in if player.SteamName is defined.
function serverguard.player:GetName(player)
	if (util.IsConsole(player)) then
		return "Console";
	elseif (type(player) == "Player" and IsValid(player)) then
		local playerName = player:Name();
		
		if (player.SteamName) then
			playerName = "(" .. player:SteamName() .. ") " .. playerName;
		end;
...
function serverguard.player:GetSpectatorTarget(player, decrease)
	if (isentity(decrease)) then
		player.spectatorIndex = player.spectatorIndex or 1;
		...
		player.spectatorIndex = player.spectatorIndex or 0;
		player.spectatorIndex = decrease and player.spectatorIndex - 1 or player.spectatorIndex + 1;
		
		if (player.spectatorIndex > #players) then
			player.spectatorIndex = 1;
		elseif (player.spectatorIndex < 1) then
			player.spectatorIndex = #players;
		end;
