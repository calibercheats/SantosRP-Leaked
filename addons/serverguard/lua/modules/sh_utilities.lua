...
function util.GetSortedPlayers()
	local playerList = {};
	local sortedPlayers = {};

	for k, v in ipairs(util.GetPlayers()) do 
		playerList[#playerList + 1] = {
			name = v:Name(), player = v
		};
	end;

	table.SortByMember(playerList, "name", function(a, b)
		return a > b
	end);

	for k, v in ipairs(playerList) do
		sortedPlayers[#sortedPlayers + 1] = v.player;
	end;
...
function util.PlayerMatchesImmunity(requiredImmunity, player, target, bNoTargetSelf)
	local result = hook.Call("serverguard.PlayerMatchesImmunity", nil, requiredImmunity, player, target, bNoTargetSelf);

	return 	result or
			(requiredImmunity == SERVERGUARD.IMMUNITY.EQUAL and serverguard.player:GetImmunity(target) == serverguard.player:GetImmunity(player)) or
			(requiredImmunity == SERVERGUARD.IMMUNITY.LESS and serverguard.player:GetImmunity(target) < serverguard.player:GetImmunity(player)) or
			(requiredImmunity == SERVERGUARD.IMMUNITY.LESSOREQUAL and serverguard.player:GetImmunity(target) <= serverguard.player:GetImmunity(player)) or
			(requiredImmunity == SERVERGUARD.IMMUNITY.ANY) or
			(!bNoTargetSelf and player == target);
end;
...
function util.ExecuteOnPlayers(identifier, callingPlayer, requiredImmunity, func)
	...
	if (#players == 0) then
		...
		for k, v in pairs(identifierTags) do
			...
			for k, player in pairs(util.GetPlayers()) do
				if (tag == text and util.PlayerMatchesImmunity(requiredImmunity, callingPlayer, player) and v[func](callingPlayer, player, argument)) then
		...
		if (#players == 0) then
			if (string.find(identifier, ",", 0, true)) then
				...
				for k, v in pairs(identifiers) do
					...
					for k2, v2 in pairs(util.GetPlayers()) do
...
function util.GetNotifyListForTargets(targets, bOwnership, targetColor)
	...
	if (#targets == #util.GetPlayers() and #targets != 1) then
		if (bOwnership) then
...
function util.ParseDuration(input)
	if (tonumber(input)) then
		if (tonumber(input) == 0 or tonumber(input) < 0) then
			return 0, "Indefinitely", false;
		end;

		local text = input .. " minute";

		if (tonumber(input) > 1) then
			text = text .. "s";
		end;

		return tonumber(input), text, false;
	end;
	...
	function util.PrintConsoleAdmins(text)
		...
		for k, player in pairs(players) do
			if (player:IsAdmin()) then
				serverguard.netstream.Start(player, "sgPrintConsole", text);
			end;
		end;
	end;
elseif (CLIENT) then
	...
	function util.CreateStringRequest(title, ...)
		...
		for k, v in pairs(arguments) do
			if (type(v) == "function") then
				if (!arguments[k + 1] or type(arguments[k + 1]) != "string") then
					continue;
				end;

				local button = dialog:Add("tiger.button");
				button:SetText(arguments[k + 1]);
		...
		function dialog:PerformLayout(width, height)
			...
			for k, v in pairs(buttons) do
				if (k == 1) then
					v:SetPos(width - (v:GetWide() + 24), height - (v:GetTall() + 14));
					lastPanel = v;

					continue;
				end;

				v:SetPos(0, height - (v:GetTall() + 14));
				v:MoveLeftOf(lastPanel, 14);
			end;
		end;
	end;
	...
	function util.CreateDialog(title, text, ...)
		...
		for k, v in pairs(arguments) do
			if (type(v) == "function") then
				if (!arguments[k + 1] || type(arguments[k + 1]) != "string") then
					continue;
				end
