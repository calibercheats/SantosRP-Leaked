...
function plugin.PhysgunPickup(player, entity)
	...
	local bCanPickup = false;
	
	if (serverguard.player:HasPermission(player, "Bypass Prop-Protection") and !entity:IsPlayer()) then
		bCanPickup = true;
	elseif ((entity:CPPIGetOwner() == game.GetWorld() or entity:CPPIGetOwner() == CPPI.CPPI_DEFER)) then
		bCanPickup = false;
	elseif (!entity:IsPlayer() and !IsValid(entity:CPPIGetOwner()) and (entity:CPPIGetOwner() != game.GetWorld() and entity:CPPIGetOwner() != CPPI.CPPI_DEFER)) then
		bCanPickup = true;
