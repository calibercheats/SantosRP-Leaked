...
local function CreateBuddiesPanel(base)
	...
	function buddiesPanel.list:Think()
		...
		for i = 1, #players do
			...
			if (!IsValid(player.propProtection) and player != LocalPlayer()) then
				local panel = self:AddItem(serverguard.player:GetName(player));
...
function category:Create(categoryBase)
	...
	categoryBase.base:AddSection("Buddies", "serverguard/menuicons/icon_mange_players.png", function(panelBase)
	...
	if (serverguard.player:HasPermission(LocalPlayer(), "Manage Prop Blacklist")) then
		categoryBase.base:AddSection("Blacklist", "serverguard/menuicons/icon_prop_protection.png", function(panelBase)
			if (!panelBase.created) then -- Create()
	...
	if (serverguard.player:HasPermission(LocalPlayer(), "Manage Prop-Protection")) then
		categoryBase.base:AddSection("Settings", "serverguard/menuicons/icon_plugins.png", function(panelBase)
			if (!panelBase.created) then -- Create()
...
function category:RemoveBlacklistProp(model, bSend)
	...
	for k, v in pairs(self.blacklistLayout:GetChildren()) do
...
serverguard.netstream.Hook("sgPropProtectionGetBlacklistChunk", function(data)
	if (type(data) == "string") then
		blacklistSendQueue[#blacklistSendQueue + 1] = data;

		serverguard.netstream.Start("sgPropProtectionRequestBlacklistChunk", true);
	elseif (type(data) == "boolean" and data == true) then
		...
		local builtData = serverguard.netstream.Build(blacklistSendQueue);

		category.blacklist.list = table.Copy(builtData);
		category.blacklist.queue = builtData;
	end;
end);