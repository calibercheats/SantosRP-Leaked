...
function PANEL:Think()
	self:SetProgress((plugin.player:GetDeltaTime() / plugin.player:GetDuration()) * 100);
end;
...
function PANEL:Paint(width, height)
	...
	local textWidth, textHeight = surface.GetTextSize(string.FormattedTime(plugin.player:GetDuration(), "%2i:%02i"));
		surface.SetTextColor(theme.tiger_base_section_label);
		surface.SetTextPos(width - textWidth - 2, 0);
	surface.DrawText(string.FormattedTime(plugin.player:GetDuration(), "%2i:%02i"));

	currentTextWidth, currentTextHeight = surface.GetTextSize(string.FormattedTime(plugin.player:GetDeltaTime(), "%2i:%02i"));
	...
	surface.DrawText(string.FormattedTime(plugin.player:GetDeltaTime(), "%2i:%02i"));
end;
...
function PANEL:Think()
	if (self.button:GetDragging()) then
		...
		if (plugin.player:GetVolume() != self:GetAmount()) then
			plugin.player:SetVolume(self:GetAmount());
		end;
	end;
end;
...
function PANEL:SetTitle(text)
	...
	self.volume:SetAmount(plugin.player:GetVolume());
