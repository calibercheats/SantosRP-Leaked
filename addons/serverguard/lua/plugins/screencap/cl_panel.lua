...
function category:Create(base)
	...
	function base.panel.list:Think()
		...
		for i = 1, #players do
			...
			if (!IsValid(player.screenPanel)) then
				local panel = base.panel.list:AddItem(serverguard.player:GetName(player), player:SteamID());
				
				panel.player = player;
				panel.unique = player:UniqueID();
				...
				function lastBase.view:DoClick()
					local data = g_ScreenCapData[panel.player:UniqueID()].data;
					local compiled = "";
					...
					baseb:Center();
					baseb:SetTitle("Screen Capture Viewer");
					baseb:MakePopup();
					...
					html:Dock(FILL);
					html:SetHTML("<img src='data:image/jpeg;base64,"..compiled.."'/>");
					...
					if (IsValid(panel.player)) then
						...
						if (data) then
							...
							if (self.ready) then
								...
								draw.SimpleText(math.Round((#data.data /data.parts) *100) .. "%", "tiger.base.footer", w -8, 8, theme.tiger_button_text, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM);
