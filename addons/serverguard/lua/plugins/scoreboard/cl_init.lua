...
function panel:AddPlayer(player)
	...
	function panel:Paint(w, h)
		if (IsValid(self.player)) then
			...
			if (IsValid(player)) then
				draw.SimpleText(serverguard.player:GetName(player), "serverguard.scoreboard.player", 62, 16, theme.tiger_base_section_label, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
				draw.SimpleText(player:Ping(), "serverguard.scoreboard.player", w -18, 16, theme.tiger_base_section_label, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
			end
		end
	end
	...
	panel.avatar:SetPlayer(player, 32)
	panel.avatar:SetCursor("hand")

	function panel.avatar:OnMouseReleased(keyCode)
		if (IsValid(player) and keyCode == MOUSE_LEFT) then
			player:ShowProfile()
		end
	end
...
function panel:Think()
	...
	for k, player in pairs(players) do
		if (IsValid(player) and !player.sg_scoreboard) then
