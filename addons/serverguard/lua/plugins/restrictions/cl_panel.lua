	...
	for i, data in ipairs(restrictionTypes) do
		...
		if (GAMEMODE.IsSandboxDerived and toolsPanel and data.type == "Tools") then
			...
			for k, v in pairs(toolsTable) do
				...
				if (restrictionData[data.name] and restrictionData[data.name][k]) then
					checkbox:SetChecked(restrictionData[data.name][k].Value);
					rankTable[i].value[k].Value = restrictionData[data.name][k].Value;
				else
