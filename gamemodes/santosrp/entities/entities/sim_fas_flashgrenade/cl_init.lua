...
function ENT:Think()
	if self.Entity:GetDTBool(0) then
		...
		if light then
			...
			light.g = 255
			light.b = 255
			light.Brightness = (GAMEMODE.Config.LightSensitiveMode:GetInt() == 1 and 0.25 or 4)
			light.Size = (GAMEMODE.Config.LightSensitiveMode:GetInt() == 1 and 128 or 1000)
			light.Decay = (GAMEMODE.Config.LightSensitiveMode:GetInt() == 1 and 3000 or 1000)
			light.DieTime = CurTime() +(GAMEMODE.Config.LightSensitiveMode:GetInt() == 1 and 3 or 0.5)
		end
	end
end
...
function FlashEffect()
	if GAMEMODE.Config.LightSensitiveMode:GetInt() == 1 then return end
