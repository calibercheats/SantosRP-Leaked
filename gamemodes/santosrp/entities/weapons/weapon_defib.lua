...
function SWEP:OnHit( ent )
	...
	if ent:IsRagdoll() then 
		...
		if not ent.RagdollPlayer:Alive() or ent.RagdollPlayer:IsUncon() then
			if hook.Call( "GamemodeCanPlayerRevivePlayer", GAMEMODE, self.Owner, ent ) ~= false then
	...
	elseif ent:IsVehicle() then
		...
		if pl.m_bStretcherWasDead or pl:IsUncon() then
			if hook.Call( "GamemodeCanPlayerRevivePlayer", GAMEMODE, self.Owner, pl ) ~= false then
				if pl:IsUncon() then
					pl:WakeUp()
				else
					--pl:UnRagdoll()
