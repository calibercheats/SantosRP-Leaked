...
if CLIENT then
    SWEP.PrintName = "Government Issue First Aid Kit"
    SWEP.Slot = 5
    SWEP.SlotPos = 0
...
if CLIENT then
	g_FAS2_PlayAnim = g_FAS2_PlayAnim or FAS2_PlayAnim
	function FAS2_PlayAnim( wep, anim, ... )
		g_FAS2_PlayAnim( wep, anim, ... )

		if CLIENT then
			if wep and wep.Wep and IsValid( wep.Wep ) then
				if type( anim ) == "string" then
					wep.Wep:ResetSequence( string.lower(anim or "") )
				else
					wep.Wep:ResetSequence( table.Random(anim) )
				end
			end
		end
	end
...
function SWEP:EndSelfHealingProcess()
	...
	if SERVER then
		local healed
		if self:CanHeal( self.Target ) and self.CurrentHeal ~= "suture" then
			local heal, left = self:HealTargetLimbs( self.Target, 30 )
			self:HealTarget( heal and math.max(left, 5) or 30 )
			healed = true
		elseif self.CurrentHeal == "suture" and GAMEMODE.PlayerDamage:IsPlayerBleeding( self.Target ) then
			if self:HealTargetBleeding( self.Target ) then
				healed = true
			end
		end
...
function SWEP:HealTargetBleeding( pPlayer )
...
function SWEP:HealTargetLimbs( pPlayer, intAmount )
	local max = GAMEMODE.Util:VectorInRange( self.Owner:GetPos(), GAMEMODE.Config.EMSHospitalZone.Min, GAMEMODE.Config.EMSHospitalZone.Max ) and 1 or 0.33
	local num = intAmount
	local cur
	for k, v in pairs( GAMEMODE.PlayerDamage:GetLimbs() ) do
...
function SWEP:CanHeal( entTarget, bSecMode )
	local max = 0.33
	...
	if SERVER then
		...
		if bSecMode then
			for k, v in pairs( GAMEMODE.PlayerDamage:GetLimbs() ) do
				if not GAMEMODE.PlayerDamage:IsPlayerLimbBleeding( entTarget, k ) then continue end
				return true
			end
		...
		if GAMEMODE.Util:VectorInRange( self.Owner:GetPos(), GAMEMODE.Config.EMSHospitalZone.Min, GAMEMODE.Config.EMSHospitalZone.Max ) then
			if GAMEMODE.PlayerDamage:PlayerHasDamagedLimbs( entTarget, true ) then
				return true
			end

			if entTarget:Health() >= entTarget:GetMaxHealth() and not GAMEMODE.PlayerDamage:PlayerHasDamagedLimbs( entTarget, true, 1 ) then
				return false
			end
		else
			...
			if entTarget:Health() >= entTarget:GetMaxHealth() *max and not GAMEMODE.PlayerDamage:PlayerHasDamagedLimbs( entTarget, true, max ) then
				self.Owner:AddNote( "You must be inside the hospital to heal any further!" )
				return false
			end
		end
	else
		if not IsValid( entTarget ) or not entTarget:IsPlayer() then return false end
		if GAMEMODE.Util:VectorInRange( self.Owner:GetPos(), GAMEMODE.Config.EMSHospitalZone.Min, GAMEMODE.Config.EMSHospitalZone.Max ) then
		--	if entTarget:Health() >= 100 then
				return true
		--	end
		else
...
function SWEP:HealTarget( amt )
	local max = 0.33
	if GAMEMODE.Util:VectorInRange( self.Owner:GetPos(), GAMEMODE.Config.EMSHospitalZone.Min, GAMEMODE.Config.EMSHospitalZone.Max ) then
		amt = math.Clamp( self.Owner:Health() +amt, 0, self.Owner:GetMaxHealth() )
	else
...
function SWEP:SecondaryAttack()
	...
	if am then
		...
		if not self:CanHeal( self.Target, true ) then
			return
		end
