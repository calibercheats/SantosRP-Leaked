...
function SWEP:DoExtinguish( pushforce, effectscale )
	...
	for id, prop in pairs( ents.FindInSphere( pos, 32 ) ) do
		if ( prop:GetPos():Distance( self:GetPos() ) < 256 ) then
			...
			if ( prop:IsValid() and math.random( 0, 1000 ) > 250 ) then
				...
				if class == "ent_fire" then
					prop:WaterHit( self.Owner )
				elseif class == "player" and prop:IsOnFire() then
					prop:Extinguish()
				end
			end
			
			if ( pushforce > 0 ) and class ~= "ent_fire" then
...
function SWEP:PrimaryAttack()
	...
	self:SetNextPrimaryFire( CurTime() + 0.166 )
end
