...
local MovieScale = 1
function GM.Camera:ToggleMovieMode() 
...
function GM.Camera:SetMovieModeSmoothing( int )
	MovieScale = int
end
...
function GM.Camera:CalcView( pPlayer, vecOrigin, angAngs, intFOV )
	...
	elseif self.m_conThirdPerson:GetInt() == 1 and (IsValid(LocalPlayer():GetActiveWeapon()) and not self.m_tblFirstPersonWeps[LocalPlayer():GetActiveWeapon():GetClass()]) then
		...
		if MAIN_MOVIEMODE then
			MovieMode_Pos = MovieMode_Pos +(vecOrigin -MovieMode_Pos) /(MovieScale /FrameTime())
			MovieMode_Ang = MovieMode_Ang +(angAngs:Forward() -MovieMode_Ang) /(MovieScale /FrameTime())
