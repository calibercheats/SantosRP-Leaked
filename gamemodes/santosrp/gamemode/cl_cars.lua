...
function GM.Cars:GetCarMaxHealth( entCar )
	if not GAMEMODE.Config.UseCustomVehicleDamage then return entCar:GetNWInt( "VC_MaxHealth", 0 ) end
	if self:GetCarByUID( entCar:GetNWString("UID", "") ) and self:GetCarByUID( entCar:GetNWString("UID", "") ).Health then
		return self:GetCarByUID( entCar:GetNWString("UID", "") ).Health
	end

	return GAMEMODE.Config.MaxCarHealth
end
