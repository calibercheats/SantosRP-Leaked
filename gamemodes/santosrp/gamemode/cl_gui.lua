...
function GM.Gui:BuildCityWorkerMenus()
	if ValidPanel( self.m_pnlCityVanMenu ) then
		self.m_pnlCityVanMenu:Remove()
	end

	self.m_pnlCityVanMenu = vgui.Create( "SRPCityWorkerVanMenu" )
	self.m_pnlCityVanMenu:SetSize( 800, 480 )
	self.m_pnlCityVanMenu:Center()
	self.m_pnlCityVanMenu:SetVisible( false )
	self:RegisterNWMenu( "city_worker_van", self.m_pnlCityVanMenu )
end
