	...
	function GM.Util:IsPointEmpty( vecPos, tblIgnore )
		...
		if tblIgnore then
			...
			for k, v in pairs( ents.FindInSphere(vecPos, 35) ) do
				if v:IsNPC() or v:IsPlayer() or v:IsVehicle() or
					v:GetClass() == "prop_physics" or v:GetClass() == "prop_physics_multiplayer" and
					not table.HasValue( tblIgnore, v ) then
...
do
	...
	local function update()
		...
		for k, v in pairs( g_TickSchedule ) do
			if v.LastTick +v.Interval < g_TickSchedule_NumTicks then
