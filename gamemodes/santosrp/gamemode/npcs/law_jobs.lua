...
NPCMeta.SubText = "Become a lawyer/prosecutor here"
...
function NPCMeta:OnPlayerTalk( entNPC, pPlayer )
	if GAMEMODE.Jobs:PlayerIsJob( pPlayer, JOB_LAWYER ) or GAMEMODE.Jobs:PlayerIsJob( pPlayer, JOB_PROSECUTOR ) then
...
function NPCMeta:PlayerBecomeLawyer( pPlayer )
	...
	if GAMEMODE.Jobs:CalcJobPlayerCap( JOB_LAWYER ) <= GAMEMODE.Jobs:GetNumPlayers( JOB_LAWYER ) and not hasFastPass then
...
function NPCMeta:PlayerBecomeProsecutor( pPlayer )
	...
	local hasFastPass = GAMEMODE.Player:GetPlayerVIPFlag( pPlayer, "job_fast_pass" ) and true
	if GAMEMODE.Jobs:CalcJobPlayerCap( JOB_PROSECUTOR ) <= GAMEMODE.Jobs:GetNumPlayers( JOB_PROSECUTOR ) and not hasFastPass then
		GAMEMODE.Net:ShowNPCDialog( pPlayer, "law_atplayercap" )
		return
	end
...
function NPCMeta:PlayerQuitJob( pPlayer )
	if not pPlayer:WithinTalkingRange() then return end
	if pPlayer:GetTalkingNPC().UID ~= self.UID then return end
	if not GAMEMODE.Jobs:PlayerIsJob( pPlayer, JOB_LAWYER ) and not GAMEMODE.Jobs:PlayerIsJob( pPlayer, JOB_PROSECUTOR ) then return end
