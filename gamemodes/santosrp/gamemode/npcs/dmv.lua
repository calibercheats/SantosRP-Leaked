...
function NPCMeta:ShowPlateMenu( pPlayer, ... )
	...
	if GAMEMODE.Player:GetPlayerVIPFlag( pPlayer, "custom_plates" ) then
...
function NPCMeta:ShowDriveTestMenu( pPlayer, ... )
	...
	if not saveTable.License or not (saveTable.License.RevokeTime or (saveTable.License.ID and saveTable.License.ID ~= "")) then
