...
Rain.m_convRainAmount = GetConVar( "srp_max_rain_particles" ) or CreateClientConVar( "srp_max_rain_particles", "25", true, false )
Rain.m_convVolume = GetConVar( "srp_rain_volume" ) or CreateClientConVar( "srp_rain_volume", "100", true, false )
Rain.m_convThunderVolume = GetConVar( "srp_rain_thunder_volume" ) or CreateClientConVar( "srp_rain_thunder_volume", "75", true, false )
...
Rain.m_tblSounds = {
	Lightning = {
		"santosrp/weather/lightning_strike_1.mp3",
		"santosrp/weather/lightning_strike_2.mp3",
		"santosrp/weather/lightning_strike_3.mp3",
		"santosrp/weather/lightning_strike_4.mp3",
	},
	Thunder = {
		[1] = {	--Close
			"santosrp/weather/thunder_close01.mp3",
			"santosrp/weather/thunder_close02.mp3",
			"santosrp/weather/thunder_close03.mp3",
			"santosrp/weather/thunder_close04.mp3",
		}, 
		[2] = {	--Kinda Close
			"santosrp/weather/thunder_1.mp3",
			"santosrp/weather/thunder_2.mp3",
			"santosrp/weather/thunder_3.mp3",
			"santosrp/weather/thunder_distant01.mp3",
			"santosrp/weather/thunder_distant02.mp3",
			"santosrp/weather/thunder_distant03.mp3",
		}, 
		[3] = {	--Distant
			"santosrp/weather/thunder_far_away_1.mp3",
			"santosrp/weather/thunder_far_away_2.mp3",
		}, 
	},
	RainLoop = "santosrp/weather/crucial_surfacerain_med_loop.wav",
}
...
function Rain:Start( intStartTime, intDuration, intTimeOffset )
	...
	if CLIENT then
		self.m_intRainMat = Material( "santosrp/weather/rain_bunch" )
		self.m_tblRainParts	= {}
...
function Rain:Indoors( pPlayer )
	...
	if pl:InVehicle() and pl:GetVehicle():GetThirdPersonMode() then return true end
	...
	function Rain:GamemodeUpdateMapLighting( strPattern, intLightIndex )
		...
		if time <= self.m_intStartTime +self.m_intFadeInTime then
			if self.m_intStartRainFadeTime then
				...
				return mod:GetLightTable()[math.ceil(diff)].Pattern
			else
				return strPattern
			end
		elseif time > self.m_intStartTime +self.m_intDurationToFadeOut then
			...
			return mod:GetLightTable()[targetLightIndex].Pattern
		end
	end
...
if CLIENT then
	...
	function Rain:FX_RainHit( vecHitPos, bHitWater )
		if bHitWater then
			bHitWater = self.m_pEmitter3D:Add( "santosrp/weather/warp_ripple", vecHitPos )
		...
		bHitWater = self.m_pEmitter:Add( "santosrp/weather/warp_ripple", vecHitPos )
		bHitWater:SetColor( 255, 255, 255, 255 )
	...
	function Rain:GamemodeSetupWorldFog( tblFogData )
		...
		tblFogData.Density = math.min( 1, Lerp(self.m_intRainScaler, 0, 0.078) +tblFogData.Density )
	end
end
