...
--[[
	...
	For: SantosRP
	By: Maw, Unknown? past devs, Ultra
]]--
...
if SERVER then
	GM.Uncon.m_tblUncon = (GAMEMODE or GM).Uncon.m_tblUncon or {}

	function pmeta:IsUncon()
		return self.m_bIsUncon
	end
	...
	function pmeta:GoUncon( bDontRagdoll, bPlayerDead )
		...
		self.m_bIsUncon = true
		GAMEMODE.Uncon.m_tblUncon[self] = true
		...
		if bPlayerDead then
			...
			self:SetNWFloat( "StartDie", CurTime() -(noMedics and 5 *60 or 0) )
		end
	...
	function pmeta:WakeUp( bNoSpawn, bKeepVars, bIgnoreHealth )
		if self:IsUncon() then
			...
			if not bKeepVars then
				...
				self.m_bIsUncon = false
				GAMEMODE.Uncon.m_tblUncon[self] = nil
				self:SetNWFloat( "StartDie", 0 )
				self.NextBeat = nil
				--self:EmitSound( "santosrp/inhale.wav", 90, 90, 1, CHAN_VOICE )
	...
	function GM.Uncon:Tick()
		for v, _ in pairs( self.m_tblUncon ) do
	...
	function GM.Uncon:EntityTakeDamage( eEnt, pDamageInfo )
		if eEnt:IsPlayer() then
			if pDamageInfo:GetDamage() >= eEnt:Health() then
				if not eEnt:IsUncon() then eEnt:GoUncon() end
				return true
			end
		elseif eEnt:IsRagdoll() then
			if IsValid( eEnt.RagdollPlayer ) and not eEnt.RagdollPlayer:IsUncon() then
				pDamageInfo:ScaleDamage( 0.1 )
	...
	local mat = Material( "santosrp/vignette_death.png", "unlitgeneric ignorez" )
