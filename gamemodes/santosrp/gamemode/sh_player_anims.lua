...
function GM.PlayerAnims:ApplyAnimID( pPlayer, strAnimID )
	...
	if not self.m_tblPlayers[pPlayer] then self.m_tblPlayers[pPlayer] = {} end
	self.m_tblPlayers[pPlayer][strAnimID] = true
...
function GM.PlayerAnims:RemoveAnimID( pPlayer, strAnimID )
	...
	if self.m_tblPlayers[pPlayer][strAnimID] then
		hook.Call( "GamemodePlayerAnimRemoved", GAMEMODE, pPlayer, strAnimID )
	end

	self.m_tblPlayers[pPlayer][strAnimID] = nil

	if table.Count( self.m_tblPlayers[pPlayer] ) <= 0 and pPlayer:GetBoneCount() then
...
function GM.PlayerAnims:GetCurrentAnimIDs( pPlayer )
	return self.m_tblPlayers[pPlayer]
end

--Updates the bones for all players
function GM.PlayerAnims:ThinkPlayerBones()
	local bone
	for v, data in pairs( self.m_tblPlayers ) do
		if not IsValid( v ) then self.m_tblPlayers[v] = nil continue end

		local boneAngs = {} --Add all anim angles
		for animID, _ in pairs( data ) do
