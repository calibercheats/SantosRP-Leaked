...
local Panel = {}
function Panel:Init()
	local modRadios = GAMEMODE.Module:GetModule( "car_radios_sh" )
	if not modRadios then return end
...
function Panel:Build()
	if self.m_tblButtons then
		for k, v in pairs( self.m_tblButtons ) do
			if v and v.IsValid and ValidPanel( v ) then v:Remove() end		
		end
	end
...
function Panel:InitializeButtons()
	...
	self.m_tblButtons = {}
	for k, v in pairs( modRadios.m_tblStations ) do
		self.m_tblButtons[#self.m_tblButtons +1] = {
			Name = v.Name,
			Cmd = function()
				modRadios:RequestPlayCarRadio( k )
			end,
		}
	end

	if GAMEMODE.Player:GetPlayerVIPFlag( LocalPlayer(), "custom_radio" ) then
		self.m_tblButtons[#self.m_tblButtons +1] = {
			Name = "Custom ID",
			Cmd = function()
				GAMEMODE.Gui:StringRequest(
					...
					"114517",
					function( text )
						modRadios:RequestPlayCustomCarRadio( tonumber(text) )
					end,
					function() end,
					"Play"
				)
