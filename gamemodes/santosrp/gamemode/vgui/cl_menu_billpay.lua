...
function Panel:SetBill( intBillID, tblBillData )
	...
	local issuePrepend = "Issue Date: ".. os.date("%X - %m/%d/%Y", self.m_tblBillData.IssueDate)
	if self.m_tblBillData.LifeTime <= 0 then
		self.m_pnlTimeLabel:SetText( issuePrepend.. ", Expiry Date: Never" )
	else
		self.m_pnlTimeLabel:SetText( issuePrepend )
	end
...
function Panel:Think()
	...
	if self.m_tblBillData.LifeTime > 0 then
		local time = GAMEMODE.Player:GetGameVar( "playtime", 0 )

		local timeStr = GAMEMODE.Util:FormatTime( math.max((self.m_tblBillData.IssueTime +self.m_tblBillData.LifeTime) -time, 0), true )
		self.m_pnlTimeLeftLabel:SetText( "Time Left: ".. timeStr )

		local timeCur = time -self.m_tblBillData.IssueTime
