...
local PAINT_COST = 1000
local SKIN_COST = 5000
local BODYGROUP_COST = 2500
local STREETGLOW_COST = 7500
	...
	for k, v in ipairs( self.m_tblOptions ) do
		...
		if v.Flag then
			v.pnl.Think = function()
				v.pnl:SetDisabled( not GAMEMODE.Player:GetPlayerVIPFlag(LocalPlayer(), v.Flag) )
			end
		end
	end
...
function Panel:Open()
	...
	g_VCModStuffs = hook.GetTable()["CalcView"]["VC_ViewCalc_Shared"]
	hook.GetTable()["CalcView"]["VC_ViewCalc_Shared"] = nil --I want my fucking camera working!
end

function Panel:Close()
	self:Remove()

	if g_VCModStuffs then
		hook.Add( "CalcView", "VC_ViewCalc_Shared", g_VCModStuffs )
	end
end
...
function Panel:Init()
	...
	self.m_pnlBtnNextSkin.DoClick = function()
		...
		local uid = car:GetNWString( "UID", "" )
		local data = GAMEMODE.Cars:GetCarByUID( uid )
		...
		if data.BlockedSkins and data.BlockedSkins[self.m_intCurrentSkin] then
			self.m_pnlBtnNextSkin:DoClick()
			return
		end
...
function Panel:BuildGroups( entVehicle )
	...
	local data = GAMEMODE.Cars:GetCarByUID( entVehicle:GetNWString("UID", "") )
	if not data then return end

	for k, v in pairs( bodyGroups ) do
		if v.num <= 1 then continue end --invalid
		if data.BlockedBodyGroups and data.BlockedBodyGroups[v.id] and type( data.BlockedBodyGroups[v.id] ) ~= "table" then
			continue
		end
		...
		if v.num <= 2 then --Add a toggle
			...
			for i = 0, v.num -1 do
				if data.BlockedBodyGroups and data.BlockedBodyGroups[v.id] then
					if table.HasValue( data.BlockedBodyGroups[v.id], i ) then
						continue
					end
				end
...
function Panel:Init()
	...
	for k, v in pairs( GAMEMODE.CarShop.m_tblUpgrades ) do
		...
		for strName, data in SortedPairsByMemberValue(v, "ID") do
			...
			pnl.Think = function()
				if data.VIP and not GAMEMODE.Player:GetPlayerVIPFlag( LocalPlayer(), "santos_customs_perf" ) then
