...
function Panel:Init()
	...
	self.m_pnlPlayBtn.DoClick = function()
		modSH:RequestPlayPropRadio( LocalPlayer():GetEyeTrace().Entity, self.m_intID )
	end
end
...
function Panel:Init()
	...
	self.m_pnlStopBtn.DoClick = function()
		modSH:RequestStopPropRadio( LocalPlayer():GetEyeTrace().Entity )
	end
...
function Panel:Refresh()
	...
	for k, v in pairs( modSH.m_tblStations ) do
	...
	if GAMEMODE.Player:GetPlayerVIPFlag( LocalPlayer(), "custom_radio" ) then
		...
		pnl.m_pnlPlayBtn.DoClick = function()
			...
			modSH:RequestPlayCustomPropRadio( LocalPlayer():GetEyeTrace().Entity, tonumber(pnl.m_pnlText:GetValue()) )
		end	
