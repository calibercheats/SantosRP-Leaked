...
function Panel:Init()
	...
	table.insert( self.m_tblSelect, self.m_pnlToolbar:AddButton("Swap View", "santosrp/phone/ic_menu_camera.png", function()
		--self.m_bFaceCam = not self.m_bFaceCam
	end) )
	table.insert( self.m_tblSelect, self.m_pnlToolbar:AddButton("Gallery", "santosrp/phone/ic_menu_gallery.png", function()
		GAMEMODE.Gui.m_pnlPhone:ShowApp( "Gallery" )
	end) )

	self.m_pnlBtnTakePic = vgui.Create( "DImageButton", self )
	self.m_pnlBtnTakePic:SetImage( "santosrp/phone/cam_button.png" )
...
function Panel:WritePicture( strData )
	...
	if not file.IsDir( "santosrp/phone/photos", "DATA" ) then
		file.CreateDir( "santosrp/phone/photos" )
	end
	if not file.IsDir( "santosrp/phone/photos/".. char_id, "DATA" ) then
		file.CreateDir( "santosrp/phone/photos/".. char_id )
	end

	local fname = os.date( "%m-%d-%Y - %X", os.time() ):gsub( ":", "_" )
	file.Write( "santosrp/phone/photos/".. char_id.. "/".. fname.. ".jpg", strData )
end
