...
function Panel:QueryDropMany( bOwnerless )
	GAMEMODE.Gui:StringRequest(
		"Drop ".. self.m_tblItem.Name,
		"Enter an amount you would like to drop",
		"1",
		function( strText )
			local num = tonumber( strText )
			if not num or num <= 0 then return end
			if num ~= num then return end
			num = math.min( num, 4000000000 )
			GAMEMODE.Net:RequestDropItem( self.m_strItemID, num, bOwnerless )
		end,
		function()
		end,
		"Drop",
